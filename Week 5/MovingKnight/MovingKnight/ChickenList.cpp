...
void ChickenList::checkBoundries(int width, int height)//checking that the chicken position have not gone pass the form for all the chicken in the list 
{
	Chicken^ index = head;
	while (index != nullptr)
	{
		index->checkBoundries(width, height);
		index = index->Next;
	}
}
